...
<div class="card" style="max-width: 800px; margin: 0 auto; padding: 32px;">
    <h2 class="section-title" style="margin-bottom: 24px;">Form Tambah Kos Baru</h2>
    ...
    <form method="POST" action="{{ route('owner.kos.store') }}" enctype="multipart/form-data">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama" class="form-input" value="{{ old('nama') }}" placeholder="Contoh: Kos Putri Melati" required>
            </div>

            <!-- Harga / Bulan -->
            <div class="form-group">
                <label class="form-label">Harga Sewa / Bulan (Rp) <span style="color:var(--danger);">*</span></label>
                <input type="number" name="harga" class="form-input" value="{{ old('harga') }}" placeholder="Contoh: 1500000" required>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" name="alamat" class="form-input" value="{{ old('alamat') }}" placeholder="Jl. Jatiwangun No. 12, Purwokerto" required>
        </div>

        <!-- Deskripsi -->
        <div class="form-group">
            <label class="form-label">Deskripsi Kos</label>
            <textarea name="deskripsi" class="form-input" rows="4" placeholder="Jelaskan kenyamanan, peraturan, dan kelebihan kos Anda...">{{ old('deskripsi') }}</textarea>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
            ...
            <div class="form-group">
                ...
                <input type="text" name="luas_kamar" class="form-input" value="{{ old('luas_kamar') }}" placeholder="Contoh: 3 x 4 m">
            </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
            ...
            <div class="form-group">
                ...
                <input type="number" name="kamar_available" id="kamar_available" class="form-input" style="display:none;" value="0">
                <input type="number" name="kamar_tersedia" class="form-input" value="{{ old('kamar_tersedia', 0) }}" min="0" required>
            </div>
            ...
            <div class="form-group" style="display: flex; flex-direction: column; justify-content: center; height: 100%;">
                <label class="form-label" style="display: flex; align-items: center; gap: 8px; cursor: pointer; margin-top: 15px;">
                    <input type="checkbox" name="is_eksklusif" value="1" {{ old('is_eksklusif') ? 'checked' : '' }} style="width: 18px; height: 18px; cursor: pointer;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); gap: 10px; padding: 12px; border: 1.5px solid var(--border); border-radius: 9px; background: var(--border-light);">
                ...
                @foreach($availableFasilitas as $fasilitas)
                    <label style="display: flex; align-items: center; gap: 6px; font-size: 13px; cursor: pointer;">
                        <input type="checkbox" name="fasilitas[]" value="{{ $fasilitas }}" {{ is_array(old('fasilitas')) && in_array($fasilitas, old('fasilitas')) ? 'checked' : '' }}>
                        {{ $fasilitas }}
                    </label>
                @endforeach
            </div>
        ...
        <!-- Foto Upload -->
        <div class="form-group">
            <label class="form-label">Foto Kos (Bisa pilih beberapa sekaligus)</label>
...
<script>
// Synchronize input fields if needed
